...
class PemecahanController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (\Auth::user()->can('manage invoice')) {
            ...
            $query->where('jenis_permohonan', 'pemecahan');
            ...
            $query = Permohonan::with('createdby')
                ->where('jenis_permohonan', 'pemecahan')
                ...
                ->addColumn('actions', function ($data) {
                    ...
                    if (Gate::check('show invoice')) {
                        $actions .= '<div class="action-btn bg-info ms-2">
                                        <a href="' . route('pemecahan.show', [Crypt::encrypt($data->id)]) . '"
                    ...
                    if (Gate::check('edit invoice')) {
                        $actions .= '<div class="action-btn bg-primary ms-2">
                                        <a href="' . route('pemecahan.edit', [Crypt::encrypt($data->id)]) . '"
                                            class="mx-3 btn btn-sm align-items-center"
                    ...
                    if (Auth::user()->can('delete invoice')) {
                        $actions .= '<div class="action-btn bg-danger ms-2">
                                        <form method="POST" action="' . route('pemecahan.destroy', $data->id) . '" id="delete-form-' . $data->id . '">
                                            ' . csrf_field() . '
                                            ' . method_field('DELETE') . '
                                            <a href="#"
    ...
    {
        $url = route('pemecahan.store');
        return view('pemecahan.create', compact('url'));
    }
    ...
    public function store(PermohonanRequest $request)
    {
        ...
        $request->merge(['jenis_permohonan' => 'pemecahan',
    ...
    public function print(string $id)
    {
        ...
        $Id = \Crypt::decrypt($id);
        $data = Permohonan::with('petugasUkur.user', 'createdby', 'kecamatan', 'desa')->find($Id);
        return view('pemecahan.print', compact('data'));
    }
    /**
    ...
    public function edit(string $id)
    {
        ...
        $url = route('pemecahan.update', $Id);
        return view('pemecahan.edit', compact('data', 'url', ));
